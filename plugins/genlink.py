...
async def incoming_gen_link(bot, message):
    ...
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URL}?Hindi_Tv_Verse={outstr}"
    else:
...
# Ask Doubt on telegram @KingVJ01
    ...
    user = await get_user(user_id)
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBWEBSITE_UR?Hindi_Tv_Verse={outstr}"
    else:
        share_link = f"https://t.me/{username}?start={outstr}"
    if user["base_site"] and user["shortener_api"] != None:
        short_link = await get_short_link(user, share_link)
...
async def gen_link_batch(bot, message):
    ...
    if " " not in message.text:
        return await message.reply("Use correct format.\nExample /batch https://t.me/Hindi_Tv_Verse/10 https://t.me/Hindi_Tv_Verse/20.")
...
# Ask Doubt on telegram @KingVJ01
    ...
    user = await get_user(user_id)
    if WEBSITE_URL_MODE == True:
        share_link = f"{WEBSITE_URLUs?Hindi_Tv_Verse=BATCH-{file_id}"
    else:
