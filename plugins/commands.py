...
def formate_file_name(file_name):
    ...
    for c in chars:
        file_name.replace(c, "")
    file_name = '@AT_SERIAL ' + ' '.join(filter(lambda x: not x.startswith('http') and not x.startswith('@') and not x.startswith('www.'), file_name.split()))
    return file_name
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('💝 𝖩𝗈ꪱᜒ𐓣 ᝪυ𝗋 Τ౿ᥣ౿𝗀𝗋ɑო 𝖢𝗁ɑ𐓣𐓣౿ᥣ', url='https://t.me/Hindi_Tv_Verse')
            ],[
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/SerialVerse_support'),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/AJ_TVSERIAL')
            ],[
